 ...
 *  sigments (а именно те, значение которых обозначено символом clicks ). Для начала записи пин строба LACHT_PIN притягивают к земле,
...
byte segments[10] = {  0b00000100, 0b00000010, 0b00000110, 0b00011110, 0b00011010, 0b00000001, 0b010000001, 0b11000011, 0b01000010, 0b00000000}; 
...
void loop() {
  int nextState;       
  switch (state) {                                      // запуск цикла состояний работы оборудования
    ...
    case STATE_CASE1:                                 // статус- прижим профиля на каретке
       ...
       sdvig(DATA_PIN,CLOCK_PIN,LATCH_PIN1, segments[clicks]);
    ...
    case STATE_CASE2:                                   // статус-включаем нагрев профиля,
          ...
          clicks = 2;                                  // нагреваем первое изделие на профиле, не пилим
          Serial.println(clicks);  
          sdvig(DATA_PIN,CLOCK_PIN,LATCH_PIN1,segments[clicks]);
          Serial.println("A");
          delay(TIME_HEAT);                                         //  ОТладить  ВРЕМЯ НАГРЕВА   
          clicks = 0;
          sdvig(DATA_PIN,CLOCK_PIN,LATCH_PIN1,segments[clicks]);  //      только нагрев
          delay(500);
    ...
    case STATE_CASE3:                              // статус-включаем нагрев профиля,опускание пилы, перфорация
          clicks = 3;
          Serial.println(clicks);  
          sdvig(DATA_PIN,CLOCK_PIN, LATCH_PIN1,segments[clicks]);      //  ОТладить   КРАЙНИМ БУДЕТ ВРЕМЯ НЕОБХОДИМОЕ НА НАГРЕВ ИЛИ ПИЛЕНИЕ ИЛИ перфорацию
          Serial.println("В");
          delay(TIME_HEAT);                                         //                                                                                 НАСТРОИТЬ
          clicks = 1;
          sdvig(DATA_PIN,CLOCK_PIN, LATCH_PIN1, segments[clicks]);  //      пила выкл, прижим нагрева выкл, перфорация выкл, профиль ЗАЖАТ!!!
          delay(500);  
    ...
    case STATE_CASE5:                                                         //статус-зажим изделия А 
          ...
          clicks = 6;
          Serial.println(clicks);  
          sdvig(DATA_PIN,CLOCK_PIN,LATCH_PIN1,segments[clicks]);            // сдвиг профиля А
          Serial.println("сдвиг изделия  А"); 
          delay(200);                                                              //время  НАСТРОИТЬ                                                                 
          clicks = 7;
          Serial.println(clicks);  
          sdvig(DATA_PIN,CLOCK_PIN,LATCH_PIN1,segments[clicks]);                       // пила2
          Serial.println("пила2"); 
          delay(SAW_TIME);                                                   // НАСТРОИТЬ время работы пилы2
               
          clicks = 8;                                                     // профиль А свободен
          Serial.println(clicks);  
          sdvig(DATA_PIN,CLOCK_PIN,LATCH_PIN1,segments[clicks]); 
          Serial.println("профиль2 свободен"); 
          delay(200);                                                          // НАСТРОИТЬ        
          clicks = 9;                                                        // все свободно
          Serial.println(clicks);  
          sdvig(DATA_PIN,CLOCK_PIN,LATCH_PIN1,segments[clicks]); 
          Serial.println("все свободно");                
          delay(PRESS_TIME); 
          Serial.print("ОБРАБОТКА ДЕТАЛИ ");Serial.print(count+1);
          Serial.println("  ЗАВЕРЩИНА");
      ...
      case STATE_CASE6:                                  // анализ по завершения операций на профиле                                              
          if (count == MAX_COUNT+1){                               //последняя,восьмая деталь обработана
...
void wait(int input, int toState, int delayMs) {
  ...
  if (!value) {                                  // при замыкании концевика                       // 
    state = toState;
  }
  delay(delayMs);
}
...
void waitSTOP(int input, int delayMs) {
  bool value = digitalRead(input);               // опрашиваем пин, указанный в скобках
  if (!value) {                                  // при замыкании концевика                       // 
    state = toState;
  }
